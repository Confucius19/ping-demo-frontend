...
export default class PieChart extends React.Component {
  ...
  render() {
    ...
    pingbursts.map((id) => {
      var value = average(id["records"].map(records => records["was_success"] ? 1 : 0)) * 100//get the average of the current ping id

      ping_reslts_map.set(id["records"][0]["dest_ip"], ping_reslts_map.has(id["records"][0]["dest_ip"]) ? (ping_reslts_map.get(id["records"][0]["dest_ip"]) + value) / 2 : value)
    })
